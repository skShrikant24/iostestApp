...
class _TestApiWidgetState extends State<TestApiWidget> {
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      _model.apiResultpz7 = await CallAllotmentCall.call(
        driverMobile: '8208687436',
      );

      if ((_model.apiResultpz7?.succeeded ?? true)) {
        _model.jsonList = await actions.parseXmlToJson(
          (_model.apiResultpz7?.bodyText ?? ''),
        );
      }
    });
        ...
        body: SafeArea(
          ...
          child: Column(
            ...
            children: [
              Builder(
                builder: (context) {
                  final list = _model.jsonList!.toList();
